<script>
	...
	let lyric;
	let song;
	let correct = answer[1].answer2.song;

	async function handleLyrics() {
		let snippet;
		fetched = await getLyrics();
		if (fetched[1].secondObject.includes('Paroles de la chanson')) {
			let fetchedAgain = getLyrics();
			lyric = await fetchedAgain;
		} else {
			lyric = fetched;
		}
		let textSplitted = lyric[1].secondObject.split(/(?=[A-Z])/);
		snippet = [textSplitted[0], textSplitted[1], textSplitted[2]];
		song = lyric[1].song;
		let distractor1 = lyric[1].distractor1;
		let distractor2 = lyric[1].distractor2;
